export const mockKitsuData = {
  data: [
    {
      id: '482',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/482',
      },
      attributes: {
        synopsis:
          'In 1950s Japan, Tatsuo Kusakabe relocates himself and his two daughters, Satsuki and Mei, to the countryside to be closer to their mother, who is hospitalized due to long-term illness. As the girls grow acquainted with rural life, Mei encounters a small, bunny-like creature in the yard one day. Chasing it into the forest, she finds "Totoro"—a giant, mystical forest spirit whom she soon befriends. Before long, Satsuki too meets Totoro, and the two girls suddenly find their lives filled with magical adventures in nature and fantastical creatures of the woods.\n[Written by MAL Rewrite]',
        description:
          'In 1950s Japan, Tatsuo Kusakabe relocates himself and his two daughters, Satsuki and Mei, to the countryside to be closer to their mother, who is hospitalized due to long-term illness. As the girls grow acquainted with rural life, Mei encounters a small, bunny-like creature in the yard one day. Chasing it into the forest, she finds "Totoro"—a giant, mystical forest spirit whom she soon befriends. Before long, Satsuki too meets Totoro, and the two girls suddenly find their lives filled with magical adventures in nature and fantastical creatures of the woods.\n[Written by MAL Rewrite]',
        titles: {
          en: 'My Neighbor Totoro',
          en_jp: 'Tonari no Totoro',
          ja_jp: 'となりのトトロ',
        },
        startDate: '1988-04-16',
        endDate: '1988-04-16',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/482/original.jpg',
        },
        coverImage: {
          original:
            'https://media.kitsu.io/anime/cover_images/482/original.jpg',
        },
      },
    },
    {
      id: '467',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/467',
      },
      attributes: {
        synopsis:
          "He's small, fluffy, and absolutely adorable: Hamtarou is one perfect little hamster! After moving to a new house with his owner, 5th grader HirokoHaruna, Hamtarou discovers other hamsters and quickly makes friends.\nThe group of hamster explorers call themselves the Ham-Hams, and there's nowhere they won't go. The Ham-Hams go on crazy adventures all around the city while their owners are away, visiting everything from plays to magical lands of candy. Meanwhile, the humans face their own dramas..\nHamtarou meets a huge cast of different personalities, gets himself into—and out of—some pretty tight spots, and even helps Hiroko and her friends out more than once. Through it all, he never stops being unbearably cute. It's Hamtarou time!",
        description:
          "He's small, fluffy, and absolutely adorable: Hamtarou is one perfect little hamster! After moving to a new house with his owner, 5th grader HirokoHaruna, Hamtarou discovers other hamsters and quickly makes friends.\nThe group of hamster explorers call themselves the Ham-Hams, and there's nowhere they won't go. The Ham-Hams go on crazy adventures all around the city while their owners are away, visiting everything from plays to magical lands of candy. Meanwhile, the humans face their own dramas..\nHamtarou meets a huge cast of different personalities, gets himself into—and out of—some pretty tight spots, and even helps Hiroko and her friends out more than once. Through it all, he never stops being unbearably cute. It's Hamtarou time!",
        titles: {
          en: 'Hamtaro',
          en_jp: 'Tottoko Hamtarou',
          ja_jp: 'とっとこハム太郎',
        },
        startDate: '2000-07-07',
        endDate: '2006-03-31',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/467/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '40788',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/40788',
      },
      attributes: {
        synopsis:
          'A warm heart of friendship extended from South Korea to Japan in wake of the 2011 Touhoku earthquake and tsunami.',
        description:
          'A warm heart of friendship extended from South Korea to Japan in wake of the 2011 Touhoku earthquake and tsunami.',
        titles: {
          en_jp: 'Tomoyo',
          ja_jp: '友よ',
        },
        startDate: '2011-08-01',
        endDate: '2011-08-01',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/40788/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '3438',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/3438',
      },
      attributes: {
        synopsis:
          "In an alternate universe to the original Clannad anime, Okazaki Tomoya is dating Sakagami Tomoyo in the 24th and final episode of Clannad.\nWhen Tomoyo is elected Student Council President, her dream to save the school's cherry blossom trees from being cut down begins to come to fruition. However, her never-ending duties and bad rumors about their relationship spread by fellow students due to Tomoya's reputation as a delinquent create a rift between the two. As Tomoya watches Tomoyo rise higher and higher into the distance, their relationship is stretched to the limit by a society that threatens to break them apart.",
        description:
          "In an alternate universe to the original Clannad anime, Okazaki Tomoya is dating Sakagami Tomoyo in the 24th and final episode of Clannad.\nWhen Tomoyo is elected Student Council President, her dream to save the school's cherry blossom trees from being cut down begins to come to fruition. However, her never-ending duties and bad rumors about their relationship spread by fellow students due to Tomoya's reputation as a delinquent create a rift between the two. As Tomoya watches Tomoyo rise higher and higher into the distance, their relationship is stretched to the limit by a society that threatens to break them apart.",
        titles: {
          en: 'Clannad: Another World',
          en_jp: 'Clannad: Mou Hitotsu no Sekai, Tomoyo-hen',
          en_us: 'Clannad: Another World, Tomoyo Chapter',
          ja_jp: '-クラナド-　もうひとつの世界　智代編',
        },
        startDate: '2008-07-16',
        endDate: '2008-07-16',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/3438/original.jpg',
        },
        coverImage: {
          original:
            'https://media.kitsu.io/anime/cover_images/3438/original.jpg',
        },
      },
    },
    {
      id: '41697',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/41697',
      },
      attributes: {
        synopsis: '',
        description: '',
        titles: {
          en_jp: 'Annyeong Totobi',
          ja_jp: '안녕 토토비',
        },
        startDate: '2011-07-20',
        endDate: '2013-09-25',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/41697/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '8315',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/8315',
      },
      attributes: {
        synopsis:
          "A Place Where There Are Moths depicts the conflict between drab concrete block apartment living and the natural environment in Japanese cities. The forces of nature are represented by the motif of a tree whose leaves metamorphose into orange moths and take over a middle-aged woman's apartment, pushing her room higher and higher within the building.\n(Source: Midnight Eye)",
        description:
          "A Place Where There Are Moths depicts the conflict between drab concrete block apartment living and the natural environment in Japanese cities. The forces of nature are represented by the motif of a tree whose leaves metamorphose into orange moths and take over a middle-aged woman's apartment, pushing her room higher and higher within the building.\n(Source: Midnight Eye)",
        titles: {
          en: 'A Place Where There Are Moths',
          en_jp: 'Ga no Iru Tokoro',
          en_us: 'A Place Where There Are Moths',
          ja_jp: '蛾のいるところ',
        },
        startDate: '2001-01-01',
        endDate: '2001-01-01',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/8315/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '5891',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/5891',
      },
      attributes: {
        synopsis:
          'Based on the original story by famous Japanese comedian George Tokoro.',
        description:
          'Based on the original story by famous Japanese comedian George Tokoro.',
        titles: {
          en_jp: 'Tonari no Tokoro',
          ja_jp: 'となりのトコロ',
        },
        startDate: '1990-05-25',
        endDate: '1990-05-25',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/5891/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '9945',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/9945',
      },
      attributes: {
        synopsis:
          'Song by the Nishi Rokugo Shonen Shōjo Gasshodan which aired in 1970 on NHK in the Minna no Uta program.',
        description:
          'Song by the Nishi Rokugo Shonen Shōjo Gasshodan which aired in 1970 on NHK in the Minna no Uta program.',
        titles: {
          en_jp: 'Torero Kamomiro',
          ja_jp: 'トレロ カモミロ',
        },
        startDate: '1970-02-01',
        endDate: '1970-02-01',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/9945/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '7013',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/7013',
      },
      attributes: {
        synopsis:
          'Tottoko Hamutaro Hai! is the sequel to Tottoko Hamutaro, or in English, Hamtaro. The series revolves around the 15 original Ham-Hams in short 5-minute episodes. The Hai! series is animated differently from the original series, most notably, the head-to-body ratio is off, and it is also computer-generated in 3-D. \n(Source: Wikipedia)',
        description:
          'Tottoko Hamutaro Hai! is the sequel to Tottoko Hamutaro, or in English, Hamtaro. The series revolves around the 15 original Ham-Hams in short 5-minute episodes. The Hai! series is animated differently from the original series, most notably, the head-to-body ratio is off, and it is also computer-generated in 3-D. \n(Source: Wikipedia)',
        titles: {
          en: 'Hamtaro Hai!',
          en_jp: 'Tottoko Hamtarou Hai!',
          en_us: 'Hamtaro Hai!',
          ja_jp: 'とっとこハム太郎は〜い!',
        },
        startDate: '2006-04-05',
        endDate: '2008-03-28',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/7013/original.jpg',
        },
        coverImage: null,
      },
    },
    {
      id: '4335',
      type: 'anime',
      links: {
        self: 'https://kitsu.io/api/edge/anime/4335',
      },
      attributes: {
        synopsis:
          'A young girl named Tomoko visits the Hiroshima Peace Memorial. There she has a mysterious meeting with Sadako, the little girl who was 2 years old when the U.S. dropped the bomb on her city during WWII and 11 when she died from the effects of radiation poisoning. Sadako takes Tomoko on an eye-opening journey.\n(Source: AniDB)',
        description:
          'A young girl named Tomoko visits the Hiroshima Peace Memorial. There she has a mysterious meeting with Sadako, the little girl who was 2 years old when the U.S. dropped the bomb on her city during WWII and 11 when she died from the effects of radiation poisoning. Sadako takes Tomoko on an eye-opening journey.\n(Source: AniDB)',
        titles: {
          en: "On A Paper Crane: Tomoko's Adventure",
          en_jp: 'Tsuru ni Notte: Tomoko no Bouken',
          en_us: "On A Paper Crane: Tomoko's Adventure",
          ja_jp: 'つるにのって とも子の冒険',
        },
        startDate: '1993-07-17',
        endDate: '1993-07-17',
        posterImage: {
          original:
            'https://media.kitsu.io/anime/poster_images/4335/original.jpg',
        },
        coverImage: null,
      },
    },
  ],
  meta: {
    count: 683,
  },
  links: {
    first:
      'https://kitsu.io/api/edge/anime?filter%5Btext%5D=totoro&page%5Blimit%5D=10&page%5Boffset%5D=0',
    next: 'https://kitsu.io/api/edge/anime?filter%5Btext%5D=totoro&page%5Blimit%5D=10&page%5Boffset%5D=10',
    last: 'https://kitsu.io/api/edge/anime?filter%5Btext%5D=totoro&page%5Blimit%5D=10&page%5Boffset%5D=673',
  },
};

export const mockKitsuDataById = {
  data: {
    id: '41204',
    type: 'anime',
    links: {
      self: 'https://kitsu.io/api/edge/anime/41204',
    },
    attributes: {
      synopsis:
        'The series follows Retsuko, a red panda office worker who unleashes her frustrations with life via death metal karaoke.',
      description:
        'The series follows Retsuko, a red panda office worker who unleashes her frustrations with life via death metal karaoke.',
      titles: {
        en: 'Aggretsuko',
        en_jp: 'Aggressive Retsuko (ONA)',
        en_us: 'Aggretsuko',
        ja_jp: 'アグレッシブ烈子',
      },
      startDate: '2018-04-20',
      endDate: '2018-04-20',
      posterImage: {
        original:
          'https://media.kitsu.io/anime/poster_images/41204/original.jpg',
      },
      coverImage: {
        original:
          'https://media.kitsu.io/anime/cover_images/41204/original.png',
      },
    },
  },
};
